...
<div class="container">
  ...
  <br/>
  <hr/>
  	<h3 class="text-center">SHA(Secure Hash Algorithm)</h3>
  	<pre><b><font size="3em" color="navy">
  		- SHA는 단방향식 암호화 기법으로(복호화 불가), 암호학적 해시함수들의 모임이다
  		일반적으로 복호화 할 수  없도록 만든 알고리즘으로, SHA-2 라고도 한다
  		해시함수가 출력되는 압축된 문장을 다이제스트(Digest)라고 하는데,
  		이때 SHA-2가 생성해주는 다이제스트의 출력길이는 256, 512Bit가 있다.
  		여기서 256Bit의 출력길이를 갖는 SHA-2의 암호화기법을 'SHA-256암호화 방식'이라고 한다.
  	</font></b></pre>
  <hr/>
