...
<p><br/></p>
	<div class="container">
		<div class="text-center">
			<button type="button" title="이전연도" onclick="location.href='Schedule.sc?yy=${yy-1}&mm=${mm}';" class="btn btn-secondary btn-sm">&lt;	&lt;</button>	
			<button type="button" title="이전월" onclick="location.href='Schedule.sc?yy=${yy}&mm=${mm-1}';" class="btn btn-secondary btn-sm">&lt;</button>	
			&nbsp;&nbsp;<font size="5px">${yy}년 ${mm+1}월</font>&nbsp;&nbsp;
			<button type="button" title="다음월" onclick="location.href='Schedule.sc?yy=${yy}&mm=${mm+1}';" class="btn btn-secondary btn-sm">&gt;</button>	
			<button type="button" title="다음년도" onclick="location.href='Schedule.sc?yy=${yy+1}&mm=${mm}';" class="btn btn-secondary btn-sm">&gt;	&gt;</button>	
			<button type="button" title="오늘날짜" onclick="location.href='Schedule.sc';" class="btn btn-secondary btn-sm">★</button>	
		</div>
		<br/>
		<div class="text-center">
			<table class="table table-border" style="height:450px;">
				...
				<tr>  <!-- 시작일 이전을 공백으로 처리! 5월의 시작요일이 수요일(4)이니 일,월,화 칸을 공백으로 처리 -->
					...
					<c:forEach begin="1" end="${lastDay}" varStatus="st">
						...
						<td id="td${cell}" ${todaySw == 1 ? 'class=today' : ''} class="text-left" style="vertical-align:top; height:120px;">
						
						<c:set var="ymd" value="${yy}-${mm + 1}-${st.count}" />
						<a href="ScheduleMenu.sc?ymd=${ymd}" class="btn btn-link">
							${st.count}<br/>
